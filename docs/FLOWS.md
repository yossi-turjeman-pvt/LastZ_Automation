...
8. **Common tab** — Claim All if present (**one** outside dismiss for reward popup), else green individual Claims (`y ≤ 0.82`, green HSV ≥ 0.20, top-of-list first). If the single dismiss click also closes the whole Gifts panel (detected via the same verification), the flow **re-opens** it before attempting the Rare switch.
...
**Entry:** menu **5** (monitor only), or menu **2** watcher with `helicopter.enabled` (priority over gifts).
