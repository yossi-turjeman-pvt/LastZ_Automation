...
| 5 | Helicopter monitor | Watches BR heli indication and runs the full Helicopter flow when spotted |
| 6 | Exit | Quit |

Menu **1** and **2** run the **same** gifts flow. Menu **2** also polls for Helicopter (Explore Treasure) and yields gifts at step boundaries when the BR heli badge appears. Menu **4** is Help-only. Menu **5** is Heli-only.
...
| `thresholds.healing_complete` | `0.58` | Healing done, collect troops (checked against every `healing_complete*.png` variant). Lower than the others - the icon has a pulsing "done" animation that live-observed dips genuine matches to ~0.65 |
...
| `trucks.allow_purple_trucks` | `false` | `false` = Refresh (+ Confirm Tips) until **orange**; `true` = purple OK too |
...
| Flag | Default | Purpose |
|------|---------|---------|
| `helicopter.enabled` | `true` | BR poll + priority preempt in menu **2**; menu **5** monitor |
